...
export default function HospitalDashboard() {
  ...
  const [mockPatients, setMockPatients] = useState<any[]>([
    { id: '12345678901', name: 'John Olusegun Doe', lastVisit: 'Today, 10:45 AM', status: 'Checked In', fetchedRecords: null },
    { id: '23456789012', name: 'Chioma Eze', lastVisit: 'May 1, 2026', status: 'Discharged', fetchedRecords: null },
    { id: '34567890123', name: 'Musa Ibrahim', lastVisit: 'April 28, 2026', status: 'Discharged', fetchedRecords: null },
  ]);
  ...
  const hospitalReports = [
    {
      id: 'r1',
      title: 'Weekly Admission Summary',
      date: 'May 5, 2026',
      type: 'PDF',
      body: 'Admissions this week: 42\nDischarges: 38\nAverage length of stay: 3.2 days\nTop departments: General Medicine, Paediatrics, Surgery.',
    },
    {
      id: 'r2',
      title: 'Diagnostic Lab Statistics',
      date: 'May 3, 2026',
      type: 'XLS',
      body: 'Lab panels completed: 186\nAbnormal flags: 24\nTurnaround under 24h: 91%\nMost ordered: CBC, Malaria RDT, HbA1c.',
    },
    {
      id: 'r3',
      ...
      date: 'April 30, 2026',
      type: 'PDF',
      body: 'SKUs audited: 312\nLow stock alerts: 14\nExpired items removed: 3\nHigh-demand: ACT, Amoxicillin, Insulin.',
    },
  ];
  ...
  */
  ...
  const downloadChart = () => {
    ...
    const lines = [
      'SELORAH HEALTH — PATIENT CHART',
      '================================',
      `Name: ${selectedPatient.name}`,
      `NIN: ${selectedPatient.id}`,
      ...
      `Blood Group: ${selectedPatient.vitals?.bloodType || 'N/A'}`,
      `Height: ${selectedPatient.vitals?.height ? selectedPatient.vitals.height + 'cm' : 'N/A'}`,
      `Weight: ${selectedPatient.vitals?.weight ? selectedPatient.vitals.weight + 'kg' : 'N/A'}`,
      `Allergies: ${(selectedPatient.allergies || []).join(', ') || 'None listed'}`,
      '',
      'Generated by Selorah Hospital Portal',
    ...
    a.href = url;
    a.download = `chart-${selectedPatient.id}-${selectedPatient.name.replace(/\s+/g, '_')}.txt`;
    a.click();
    URL.revokeObjectURL(url);
    // Also open print dialog for clean paper copy
  ...
  const downloadAllReports = () => {
    hospitalReports.forEach((r, i) => {
      setTimeout(() => downloadReport(r), i * 200);
    });
  };
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-[70px] bg-transparent flex items-center justify-between px-4 md:px-8 lg:px-12 shrink-0 z-10">
          ...
          <div className="flex items-center gap-6">
            <form onSubmit={handleSearchSubmit} className="relative hidden md:block w-[320px]">
          ...
          <Routes>
                  ...
                  <div className="flex items-center justify-between mb-6 print:hidden gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-2 bg-[#101217] text-white px-5 py-2 rounded-xl font-bold text-sm hover:bg-black transition-all"
                      >
                        Download Chart
                      </button>
                      <button type="button" className="flex items-center gap-2 bg-[#6183FF]/10 text-[#6183FF] px-5 py-2 rounded-xl font-bold text-sm hover:bg-[#6183FF] hover:text-white transition-all">
                ...
                <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 animate-in fade-in slide-in-from-left-4 duration-500">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-gray-500">
                      ...
                      <tbody>
                        {(searchResultsMode ? filteredPatients : allPatients).map((patient) => (
            ...
            <Route path="reports" element={
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                {selectedReport ? (
                  <div>
                    <button type="button" onClick={() => setSelectedReport(null)} className="text-sm font-bold text-gray-400 hover:text-[#6183FF] mb-6 flex items-center gap-2">
                    ...
                    <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
                      <div>
                        ...
                        <p className="text-sm text-gray-400">{selectedReport.date} · {selectedReport.type}</p>
                      </div>
                      <button type="button" onClick={() => downloadReport(selectedReport)} className="bg-[#6183FF] text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-[#4E6EEF]">
                        Download
                      </button>
                    </div>
                    <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 whitespace-pre-wrap text-sm text-gray-700 leading-relaxed font-medium">
                      {selectedReport.body}
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
                      ...
                      <button type="button" onClick={downloadAllReports} className="bg-gray-100 text-gray-600 px-5 py-2 rounded-xl text-sm font-bold hover:bg-gray-200 transition-all">
                        Download All
                      </button>
                    </div>
                    <div className="space-y-4">
                      {hospitalReports.map((report) => (
                        <div
                          ...
                          className="flex items-center justify-between p-5 border border-gray-50 rounded-2xl hover:bg-gray-50 transition-all cursor-pointer"
                        >
                          <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              className="text-xs font-bold text-[#6183FF] hover:underline px-3 py-1.5"
                            >
                              Download
                            </button>
